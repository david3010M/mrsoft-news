{{-- Override de MoonShine 2.0: BelongsToMany en modo select.
     Cambios respecto al original:
       · El botón "Agregar" (creatable) va inline y compacto, alineado a la
         derecha sobre el select — ya no como bloque grande con divider.
       · Sin <x-moonshine::divider/> (ocupaba demasiado espacio vertical). --}}
<div x-id="['belongs-to-many']" :id="$id('belongs-to-many')" data-field-block="{{ $element->column() }}">
    @if($element->isSelectMode())
        @if($element->isCreatable())
            <div class="btm-creatable-inline">
                {{
